...
void main()
{
    ...
    vec2 uv = getLightSpaceUV(v_PosW);
    float depth = texture(u_Texture0, uv).r;
    float bias = 0.005;
    float visibility = (depth - bias > length(u_LightPos - v_PosW)) ? 0.0 : 1.0;
